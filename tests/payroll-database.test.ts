...
test(
  ...
  async () => {
    ...
    try {
      ...
      await sql`INSERT INTO overtime_claims (id,organisation_id,employee_id,date,hours,cost_centre_id,activity_code_id,location_id,reason,request_kind,emergency_reason,compensation_type,status,approved_at,approved_by,created_by,updated_by) VALUES (${ids.overtime},${ids.org},${ids.employee},'2026-09-05',4,${ids.costCentre},${ids.activity},${ids.location},'Month-end work','Emergency Retrospective','Month-end work','Payment','Approved','2026-09-06',${ids.adminUser},${ids.employeeUser},${ids.adminUser})`;
      await sql`INSERT INTO travel_requests (id,organisation_id,employee_id,purpose,destination,start_date,end_date,total_estimate,currency,status,manager_approval_status,hr_approval_status,accounts_approval_status,pre_authorised_at,actual_total,actual_total_omr,closed_at,closed_by,created_by,updated_by) VALUES (${ids.travel},${ids.org},${ids.employee},'Client visit','Muscat','2026-08-01','2026-08-03',100,'OMR','Closed','Approved','Approved','Approved','2026-07-20',90,90,'2026-09-07',${ids.adminUser},${ids.employeeUser},${ids.adminUser})`;
