...
import { verifyServerActorRole } from "../src/lib/db/utils.server.ts";

const testDbUrl =
  process.env["VIA_HR_TEST_DATABASE_URL"] ||
  process.env["DATABASE_URL"] ||
  "postgresql://via_hr_app:replace-with-a-local-only-password@localhost:55432/via_hr";

process.env["DATABASE_URL"] = testDbUrl;

test("H3.5A PostgreSQL master data and settings transactions with atomic audit logging", async () => {
