...
import { AlertTriangle, CheckCircle, Paperclip, XCircle } from "lucide-react";
...
function OvertimeApprovalsRoute() {
  ...
  const [claims, setClaims] = useState<OvertimeClaim[]>(() => otService.getClaimsForDirectReports(actorContext));
  ...
  const myDirectReports = allEmployees.filter((e) => e.lineManagerId === currentUser.employeeId).map((e) => e.id);
  const pendingManager = claims.filter((c) => c.status === "Pending Manager" && myDirectReports.includes(c.employeeId));
    ...
    >
      <div className="flex flex-col gap-6 max-w-[1200px] mx-auto pb-10">
                ...
                <Table>
                  ...
                  <TableBody>
                            ...
                            {c.crossCheckWarnings && c.crossCheckWarnings.length > 0 ? (
                              <div className="flex flex-col gap-1">
                                {c.crossCheckWarnings.map((w: string, i: number) => (
                                  <div key={i} className="flex items-start text-xs text-amber-600 bg-amber-50 p-1 rounded">
                              ...
                              <Badge variant="outline" className="text-emerald-600 border-emerald-200">Verified Match</Badge>
                            )}
                          </TableCell>
                          <TableCell className="text-right whitespace-nowrap">
                            {c.evidenceFileId && (
                              <Button variant="ghost" size="sm" onClick={() => void viewEvidence(c.id)}>
                                <Paperclip className="w-4 h-4" />
                              </Button>
                            )}
                            <Button variant="ghost" size="sm" onClick={() => handleOpenReject(c)} className="text-destructive">
                    ...
                    {pendingManager.length === 0 && (
                      <TableRow>
                        <TableCell colSpan={6} className="text-center py-8 text-muted-foreground">No pending requests for your team.</TableCell>
                      </TableRow>
                    )}
                  </TableBody>
                </Table>
                        ...
                        return (
                          <TableRow key={c.id}>
                            ...
                            <TableCell className="text-right whitespace-nowrap">
                              ...
                              <Button variant="ghost" size="sm" onClick={() => handleOpenHrAction(c, "reject")} className="text-destructive">
                                <XCircle className="w-4 h-4 mr-1" /> Reject
                              </Button>
        ...
        <Dialog open={rejectDialogOpen} onOpenChange={setRejectDialogOpen}>
          <DialogContent>
            ...
            <DialogFooter>
              ...
              <Button variant="destructive" onClick={handleReject} disabled={rejectReason.trim().length < 3}>Confirm Rejection</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
        ...
        <Dialog open={!!hrSelectedClaim} onOpenChange={(o) => !o && setHrSelectedClaim(null)}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{hrActionType === "approve" ? "Verify Overtime" : "Reject Overtime"}</DialogTitle>
              <DialogDescription>
            ...
            <div className="space-y-4 text-sm">
              <div className="grid grid-cols-2 gap-2 bg-muted/30 p-3 rounded-md">
                <div><span className="text-muted-foreground">Date:</span> {hrSelectedClaim?.date}</div>
                <div><span className="text-muted-foreground">Hours:</span> {hrSelectedClaim?.hours}h</div>
                <div className="col-span-2"><span className="text-muted-foreground">Employee Explanation:</span> {hrSelectedClaim?.reason}</div>
                {hrSelectedClaim?.crossCheckWarnings && hrSelectedClaim.crossCheckWarnings.length > 0 && (
                ...
                {hrSelectedClaim?.evidenceFileId && (
                  <div className="col-span-2">
                    <Button variant="outline" size="sm" onClick={() => void viewEvidence(hrSelectedClaim.id)}>
              ...
              <div className="space-y-2">
                ...
                <Textarea value={hrNotes} onChange={(e) => setHrNotes(e.target.value)} placeholder="Optional notes regarding decision..." />
              </div>
            </div>
            <DialogFooter className="flex gap-2">
              <Button variant="outline" onClick={() => setHrSelectedClaim(null)}>Cancel</Button>
              <Button variant={hrActionType === "approve" ? "default" : "destructive"} onClick={handleHrFinalise}>
