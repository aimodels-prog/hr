...
function HrTravelApprovalsRoute() {
  ...
  const pendingHr = requests.filter(r => r.hrApprovalStatus === "Pending" && r.status !== "Withdrawn");
    ...
    <RequirePermission permission="travel:hr_review" resourceName="HR Travel Approvals">
      <div className="flex flex-col gap-6 max-w-[1200px] mx-auto pb-10">
              ...
              <CardContent className="p-0">
                <Table>
                  ...
                  <TableBody>
                    {pendingHr.map(r => {
                      ...
                      return (
                        <TableRow key={r.id}>
                          ...
                          <TableCell className="whitespace-nowrap">{r.startDate} to {r.endDate}</TableCell>
                          <TableCell className="max-w-[250px] truncate text-sm" title={r.purpose}>{r.purpose}</TableCell>
                          <TableCell>
                    ...
                    {pendingHr.length === 0 && (
                      <TableRow>
                        <TableCell colSpan={6} className="text-center py-8 text-muted-foreground">No pending HR travel reviews.</TableCell>
                      </TableRow>
                    )}
                  </TableBody>
                </Table>
              ...
              <CardContent className="p-0">
                <Table>
                  ...
                  <TableBody>
                    {processed.map(r => {
                      ...
                      return (
                        <TableRow key={r.id}>
                          ...
                          <TableCell className="whitespace-nowrap">{r.startDate} to {r.endDate}</TableCell>
                          <TableCell>
                             <Badge variant={r.hrApprovalStatus === "Approved" ? "default" : "destructive"}>{r.hrApprovalStatus}</Badge>
                          </TableCell>
                          <TableCell>
                    ...
                    {processed.length === 0 && (
                      <TableRow>
                        <TableCell colSpan={5} className="text-center py-8 text-muted-foreground">No processed requests.</TableCell>
                      </TableRow>
                    )}
                  </TableBody>
                </Table>
        ...
        <Dialog open={!!selectedReq} onOpenChange={(o) => !o && setSelectedReq(null)}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{actionType === "approve" ? "Approve Travel Policy" : "Reject Travel Policy"}</DialogTitle>
              <DialogDescription>
                {actionType === "approve" 
            ...
            <div className="space-y-4 text-sm">
              <div className="grid grid-cols-2 gap-2 bg-muted/30 p-3 rounded-md">
                <div><span className="text-muted-foreground">Destination:</span> {selectedReq?.destination}</div>
                <div><span className="text-muted-foreground">Dates:</span> {selectedReq?.startDate} to {selectedReq?.endDate}</div>
                <div className="col-span-2"><span className="text-muted-foreground">Purpose:</span> {selectedReq?.purpose}</div>
                <div className="col-span-2"><span className="text-muted-foreground">Notes:</span> {selectedReq?.notes || "None"}</div>
              </div>
              <div className="space-y-2">
                <label className="font-medium">HR Review Notes {actionType === "reject" && <span className="text-destructive">*</span>}</label>
