...
function TravelClosuresRoute() {
  return (
    <RequirePermission permission="travel:final_close" resourceName="Travel Closures">
      <TravelClosuresContent />
    </RequirePermission>
  );
}

function TravelClosuresContent() {
              ...
              <Table>
                ...
                <TableBody>
                  {pendingClosure.map((r) => {
                    ...
                    const authorisedEstimate = r.authorisedBudget?.totalEstimate ?? r.totalEstimate;
                    ...
                    return (
                      <TableRow key={r.id}>
                        ...
                        <TableCell className="text-muted-foreground">
                          {authorisedEstimate.toLocaleString()}{" "}
                          {r.authorisedBudget?.currency ?? r.currency}
                        </TableCell>
                        <TableCell className="font-bold">
                          {r.actualTotal?.toLocaleString()} {r.currency}
                          {r.actualTotalOmr !== undefined && r.currency !== "OMR" && (
                            <span className="block text-xs font-normal text-muted-foreground">
                              {r.actualTotalOmr.toLocaleString()} OMR equivalent
                            </span>
                          )}
                        </TableCell>
                        <TableCell>
          ...
          <div className="space-y-4 text-sm">
            <div className="grid grid-cols-2 gap-2 bg-muted/30 p-3 rounded-md">
              <div className="col-span-2 text-lg font-bold pb-2 border-b">
                Actual Claimed: {selectedReq?.actualTotal?.toLocaleString()} {selectedReq?.currency}{" "}
                <span className="text-sm font-normal text-muted-foreground ml-2">
                  (Pre-authorised:{" "}
                  {(
                    selectedReq?.authorisedBudget?.totalEstimate ?? selectedReq?.totalEstimate
                  )?.toLocaleString()}{" "}
                  {selectedReq?.authorisedBudget?.currency ?? selectedReq?.currency})
                </span>
              </div>
              {selectedReq?.actualTotalOmr !== undefined && selectedReq.currency !== "OMR" && (
                <div className="col-span-2 text-muted-foreground">
                  Payroll equivalent: {selectedReq.actualTotalOmr.toLocaleString()} OMR
                </div>
              )}
