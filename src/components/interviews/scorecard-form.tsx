...
import type { InterviewScorecard, InterviewTemplate, CriterionScore, ScorecardRecommendation } from "@/lib/data/types";
...
export function ScorecardForm({ open, onOpenChange, interviewId, templateId, targetPanelUserId, onSuccess }: ScorecardFormProps) {
  ...
  const handleScoreChange = (criterionId: string, score: number) => {
    if (!canEdit) return;
    setScores(prev => ({ ...prev, [criterionId]: { ...prev[criterionId], score, evidence: prev[criterionId]?.evidence || "" } }));
  };

  const handleEvidenceChange = (criterionId: string, evidence: string) => {
    if (!canEdit) return;
    setScores(prev => ({ ...prev, [criterionId]: { ...prev[criterionId], score: prev[criterionId]?.score || 0, evidence } }));
  };
  ...
  const handleSaveDraft = () => {
    if (!scorecard) return;
    try {
      scorecardService.saveDraft(scorecard.id, buildScoresArray(), (recommendation as ScorecardRecommendation) || null, currentUser!.getActorContext());
  ...
  const handleSubmit = () => {
    ...
    try {
      scorecardService.submitScorecard(scorecard.id, buildScoresArray(), recommendation as ScorecardRecommendation, currentUser!.getActorContext());
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <div className="flex justify-between items-start">
            ...
            {scorecard?.status === "Submitted" && <Badge className="bg-emerald-500">Submitted</Badge>}
            {scorecard?.status === "Draft" && <Badge variant="outline">Draft</Badge>}
          </div>
        </DialogHeader>

        {(!scorecard && currentUser!.userId !== panelUserId) ? (
          <div className="py-8 text-center text-muted-foreground">This panel member has not started their scorecard yet.</div>
        ) : (
          <div className="space-y-6 my-4">
            {template.blindScoring && canEdit && (
              <Alert className="bg-blue-50 text-blue-900 border-blue-200">
                ...
                <AlertDescription>
                  <strong>Blind Scoring:</strong> Your scores are hidden from other panel members until everyone has submitted.
                </AlertDescription>
              </Alert>
            )}
              ...
              {template.criteria.map(c => (
                <div key={c.id} className="p-4 border rounded-md bg-muted/20">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <Label className="mb-2 block">Evidence {c.requiresEvidence && <span className="text-destructive">*</span>}</Label>
                      <Textarea 
                        value={scores[c.id]?.evidence || ""} 
                        onChange={(e) => handleEvidenceChange(c.id, e.target.value)}
                        placeholder={c.requiresEvidence ? "Required evidence..." : "Optional notes..."}
                        disabled={!canEdit}
                      />
                    </div>
                  </div>
            ...
            <div className="p-4 border rounded-md bg-primary/5 border-primary/20">
              <Label className="text-lg mb-4 block">Overall Recommendation <span className="text-destructive">*</span></Label>
              <div className="flex gap-3">
                {["Strong Yes", "Yes", "Unsure", "No"].map(rec => (
                  <Button 
                    ...
                    disabled={!canEdit}
                    className={recommendation === rec ? (rec.includes("Yes") ? "bg-emerald-600 hover:bg-emerald-700" : rec === "No" ? "bg-destructive hover:bg-destructive/90" : "bg-amber-600 hover:bg-amber-700") : ""}
                  >
                    {rec}
                  </Button>
                ))}
            ...
            {scorecard?.revisionHistory && scorecard.revisionHistory.length > 0 && (
              <div className="mt-8 space-y-4">
                <h4 className="font-semibold flex items-center gap-2"><AlertTriangle className="h-4 w-4 text-amber-500" /> Revision History</h4>
                {scorecard.revisionHistory.map((rev, idx) => (
                  <div key={idx} className="p-3 border rounded text-sm bg-muted/30">
                    <div className="flex justify-between text-muted-foreground mb-1">
                      <span>{new Date(rev.date).toLocaleString()} by {rev.actor}</span>
                    </div>
                    <div><strong>Reason:</strong> {rev.reason}</div>
                  </div>
                ))}
              </div>
            )}
            ...
            {isReadOnly && isAdmin && (
              <div className="border-t pt-6 mt-6">
                {!isReopening ? (
                  <Button variant="destructive" onClick={() => setIsReopening(true)}>Reopen for Correction</Button>
                ) : (
                  <div className="space-y-4 p-4 border border-destructive/20 bg-destructive/5 rounded-md">
                    ...
                    <Input value={reopenReason} onChange={(e) => setReopenReason(e.target.value)} placeholder="Must provide a reason for audit log..." />
                    <div className="flex gap-2">
                      <Button variant="outline" onClick={() => setIsReopening(false)}>Cancel</Button>
                      <Button variant="destructive" onClick={handleReopen} disabled={reopenReason.trim().length < 5}>Confirm Reopen</Button>
                    </div>
                  </div>
                )}
              </div>
